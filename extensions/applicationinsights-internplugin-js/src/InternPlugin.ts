...
import { ObjectToElements } from './components/helpers';
import { tempStyle, permStyle } from './components/styleNodeSrc';
import { debugBin } from './components/debugBins';
...
export default class InternPlugin extends BaseTelemetryPlugin {
   ...
   * the logging element
   */
  public myLogger: HTMLDivElement;
  ...
  initialize(config: IConfiguration, core: IAppInsightsCore, extensions: IPlugin[], pluginChain?: ITelemetryPluginChain) {
    ...
    for (let i = 0; i < this.trackers.length; i++) {
      ...
      this.debugBins[tracker] = new debugBin(tracker, 0, debugBinContainer, i * 50);
    ...
    const loggerEl = this.myLogger = document.createElement("div");
    loggerEl.className = 'my-logger';
  ...
  preProcessItem(itemType: string) {
    return (funcArgs: IInstrumentCallDetails, ...orgArgs: any[]) => {
      ...
      this.myLogger.appendChild(ObjectToElements(funcArgs, `${new Date()} ${itemType}`, 0));
  ...
  processTelemetry(event: ITelemetryItem, itemCtx?: IProcessTelemetryContext) {
    ...
    this.myLogger.appendChild(ObjectToElements(event, `${event.time} ${event.baseType}`, 0));
  }
}